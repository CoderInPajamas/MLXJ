...
| 本地检查点 | 严格正确 | 错误动作选择 / 全部请求 | 同页面新话语 p50 / p95 |
| :--- | ---: | ---: | ---: |
...
- **145 项核心测试通过**：请求约定、状态更新、过期结果、单次执行授权、HTTP 等。它们不等于模型语义准确率。
- **112 / 112 缓存比较通过**：两个模型各 28 场景 × 2 种复用条件，和全新计算对照。
- **浏览器记录完成 16 个场景检查**：包含真实 DOM 点击与执行回执；拒绝场景允许两种拒绝状态，标准与上面的严格质量集不同。

Qwen 的 3 个未通过场景包括拒绝状态区分和排序后的选项并列。GLM 出现错误动作，**当前不建议用它自动执行操作**。28 条样本不能证明模型不会选错，也不足以证明广泛可靠性。

<details>
<summary><strong>我们具体怎么测？点击展开</strong></summary>

1. 从零编写 **16 条开发场景 + 28 条冻结测试场景**，不导出任何生产数据；首版只验证英文输入。
2. 在开发集调整通用提示词，然后冻结语义提示词与拒绝阈值。测试集不用于调阈值。
3. 比较同模型的直接候选评分、只生成一个选项编号、生成结构化 JSON；保留原始失败输出。
