...
这是 JEV MLX 虚构桌面 Morrow Studio 的一次连续真实模型录制：八条预先确定的英文请求、四次明确标注的手动准备操作，以及模型选中后实际发生的 DOM 点击。它用于展示集成方式，不是独立的质量或性能 benchmark。

录像画面中的 **MLXJ** 是项目现名 JEV MLX 之前的名称。本次更名没有重新录制，也没有修改原始影像或记录。
