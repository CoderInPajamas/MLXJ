...
JEV MLX Blocks 是一个由本地模型选择方块落点的游戏。每次选择都能在棋盘上直接看到：
方块逐渐堆叠，填满的行被消除，不好的连续选择也可能导致游戏结束。
这是原创的简化规则实现，不是官方俄罗斯方块产品，也不实现竞技俄罗斯方块的完整规则。
本演示没有复制第三方游戏代码。

已有录像中的 **MLXJ** 是更名前的名称。本次更名没有重新录制游戏，
原始视频和决策记录保持不变。

## 本地运行

按照[快速开始](../README.zh-CN.md#quick-start)在项目独立环境中安装 JEV MLX，
然后使用已有的兼容本地模型启动服务：
...
打开 [JEV MLX 方块游戏](http://127.0.0.1:8765/blocks)。可以启动本地模型连续游玩、
...
它只提供候选，由 JEV MLX 使用配置的本地模型为候选的单 token 编码评分。
