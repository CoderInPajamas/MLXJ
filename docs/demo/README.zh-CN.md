...
页面展示公开[浏览器记录](../assets/browser-demo/browser-transcript.json)中的全部 16 个场景及独立的状态变化检查。原始英文话语、候选 ID、logits、分数、实测耗时、缓存信息和执行回执都予以保留。界面语言随主页入口确定：中文主页进入中文回放，英文主页进入英文回放。语言参数为 `?lang=en` 或 `?lang=zh-CN`；不带语言参数直接打开时默认中文。语言只影响界面，不翻译原始输入或结果；缺失字段仍保持缺失。
