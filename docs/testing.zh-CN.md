...
[English: evaluation protocol](evaluation.md) · [English: full results](results.md) · [新增评测协议](extended-evaluation-protocol.md) · [Gemma 实测](gemma4-results.md) · [支持模型](models.md)

最新核心测试 **161 项通过**。新增 Gemma 在原有 28 题上为 **26/28**，但包含一次筛选后首项选择错误，即 **1/26 个 enum 请求返回错误动作**，详见[Gemma 报告](gemma4-results.md)。六领域扩展的 36 题仍在运行，尚不宣称其结果。
...
**动作误选率**只看 enum 请求：错误动作选择数除以 26 个 enum 请求；**动作覆盖率**是正确动作选择数除以 16 个应可执行 enum 请求。模型评测本身不执行这些动作。布尔题单独统计答案准确率，不能把事实答案称为执行动作。
